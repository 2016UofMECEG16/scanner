 ...
 ******************************************************************************/
 ...
 /* wiringPI library used for LEDs: http://wiringpi.com/*/
#include <stdio.h>
#include <stdlib.h>
#include <ctype.h>
#include <string.h>
		...
		if(eDevState_PRESENT == g_DevState)
		{
			...
			if(eDevType_TAG == g_Dev_Type)
			{
				...
				else
				{
					if (0x05 == mode)
					{
						...
						if ((dev == '1') && (valid == '1') && (cidfound))
						{
							...
							if (diffTime > TRANSFERSEC || TRANSFERSOFF)
							{
								...
								strToFloat(bal, &balf);
								printf("balf: %f\n", balf);
								if(balf < 0) //allow one transaction to bring user to negative balance
								{
									printf("Invalid, not enough balance for transaction");
									#ifdef LEDON
	        							blink(RED, 2);
					...
					{
						printf("Error: can't use HCE in kiosk mode\n");
						#ifdef LEDON
	        				blink(RED, 2);
						#endif /* LEDON */
					}
